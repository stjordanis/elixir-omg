...
defmodule OMG.API.State.TransactionTest do
  ...
  test "checks if spenders are authorized", %{alice: alice, bob: bob, carol: carol} do
    ...
    :ok = Transaction.Recovered.check_spenders_authorized(alice_authorized_tx, [bob.addr, alice.addr])
    {:error, :unauthorized_spent} = Transaction.Recovered.check_spenders_authorized(alice_authorized_tx, [carol.addr])

    {:error, :unauthorized_spent} =
      Transaction.Recovered.check_spenders_authorized(alice_authorized_tx, [alice.addr, carol.addr])
  end
end
